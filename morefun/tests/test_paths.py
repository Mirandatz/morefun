...
def test_grammars_dir() -> None:
    path = morefun.paths.get_grammars_dir()
...
def test_get_generation_checkpoint_path(generation_nr: int) -> None:
    ...
    actual = morefun.paths.get_generation_checkpoint_path(
        base_output_dir, generation_nr
    )
...
def test_get_latest_generation_output_path(generation_numbers: list[int]) -> None:
    with tempfile.TemporaryDirectory(dir="/dev/shm") as _temp_dir:
        ...
        for gen_nr in generation_numbers:
            morefun.paths.get_generation_checkpoint_path(temp_dir, gen_nr).write_text(
                ""
            )

        expected = morefun.paths.get_generation_checkpoint_path(
            temp_dir, max(generation_numbers)
        )
        actual = morefun.paths.get_latest_generation_checkpoint_path(temp_dir)
        assert expected == actual
